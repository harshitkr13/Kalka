...
export default function AdminIndustriesPage() {
  ...
  // Delete modal state
  const [deletingItem, setDeletingItem] = useState<IndustryItem | null>(null);
  ...
  const loadIndustries = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetchAdminList<IndustryItem>('industries', {
        page,
        limit: 15,
      ...
      if (res.success && res.data) {
        ...
        setError(res.error || 'Failed to load industries directory');
      }
    } catch {
      setError('A network error occurred while retrieving industries');
    } finally {
      setLoading(false);
    }
  }, [page, search, status]);
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await deleteAdminItem('industries', deletingItem._id);
      if (res.success) {
      ...
      alert('Network error occurred while deleting industry');
    } finally {
      setIsDeleting(false);
    }
  };

  const columns: Column<IndustryItem>[] = [
    {
      key: 'name',
  ...
  return (
    <div>
      ...
      <ContentTable
        ...
        emptyDescription="Define your firm's industry practice areas or adjust search filters."
        emptyAction={
          <Link
